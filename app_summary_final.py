...
try:
    from expression_layer import generate_role_adaptive_thematic_summary
except Exception:
    generate_role_adaptive_thematic_summary = None
...
def generate_basic_thematic_summary(summary_df, code_counts=None, topic_df=None, max_segments=40):
    """Generate a lightweight qualitative thematic summary from coded outputs.

    This layer is intentionally placed after coding/evaluation. It does not alter
    labels, metrics, RAG retrieval, or human comparison. It only turns existing
    coded evidence into a short research-facing interpretation.
    """
    if summary_df is None or summary_df.empty:
        return "No coded segments were available for thematic summary generation."

    coded = summary_df.copy()
    coded["codes"] = coded.get("codes", "").fillna("").astype(str)
    coded = coded[coded["codes"].str.strip() != ""]

    if coded.empty:
        return "No non-empty coded segments were available for thematic summary generation."

    segment_lines = []
    for _, row in coded.head(max_segments).iterrows():
        text = str(row.get("text", "")).strip().replace("\n", " ")
        codes = str(row.get("codes", "")).strip()
        if text:
            segment_lines.append(f"Codes: {codes} | Text: {text[:500]}")

    topic_text = ""
    if topic_df is not None and not topic_df.empty:
        topic_items = []
        for _, row in topic_df.head(6).iterrows():
            topic_items.append(f"Topic {row.get('topic_id', '')}: {row.get('top_words', '')}")
        topic_text = "\n".join(topic_items)

    count_text = ""
    if code_counts is not None and not code_counts.empty:
        count_text = ", ".join([f"{code}={int(count)}" for code, count in code_counts.items()])

    prompt = f"""
You are assisting with qualitative thematic analysis for an ENVIO interview.

User's analysis question or coding instruction:
{task_query}
...
Representative coded transcript excerpts:
{chr(10).join(segment_lines)}

Write a concise qualitative thematic summary in 3-5 short paragraphs.
Focus on:
- major challenges discussed in the interview
- support needs and support systems
- healthcare access issues
- institutional or environmental barriers
- stigma or mental health themes when supported by the excerpts

Do not invent facts beyond the coded excerpts.
Use a natural research-summary tone.
"""
    ...
    try:
        response = client.chat.completions.create(
            ...
            temperature=0.2,
            messages=[
                {"role": "system", "content": "You are a careful qualitative research assistant."},
                {"role": "user", "content": prompt},
            ],
        )
        ...
        fallback = []
        if count_text:
            fallback.append(f"The most frequent qualitative codes were: {count_text}.")
        if topic_text:
            fallback.append(f"Topic modeling highlighted: {topic_text}")
        fallback.append(f"Thematic summary generation was skipped because of: {e}")
        return "\n\n".join(fallback)


def apply_role_adaptive_summary(thematic_summary, profile, query_text=""):
    """Optionally rewrite the thematic summary for the selected role.

    Uses expression_layer.py when available. Falls back to the original summary
    without affecting the rest of the app.
    """
    if not thematic_summary or generate_role_adaptive_thematic_summary is None:
        return thematic_summary

    try:
        return generate_role_adaptive_thematic_summary(
...
if st.button("Run"):
    ...
    if mode == "coding":
        ...
        if all_outputs:
            ...
            # Lightweight thematic summary layer.
            # This does not modify coding outputs, RAG comparison, or human-evaluation metrics.
            try:
                st.subheader("Qualitative Thematic Summary")
                thematic_summary = generate_basic_thematic_summary(
                    summary_df=summary_df,
                    code_counts=code_counts,
                    topic_df=topic_df,
                )
                adaptive_thematic_summary = apply_role_adaptive_summary(
                    thematic_summary=thematic_summary,
                    profile=profile,
                    query_text=task_query,
                )
                st.write(adaptive_thematic_summary)

                thematic_summary_file = os.path.join(user_output_dir, "qualitative_thematic_summary.txt")
                with open(thematic_summary_file, "w", encoding="utf-8") as f:
                    f.write(adaptive_thematic_summary)
                generated_files.append(thematic_summary_file)
            except Exception as e:
                st.warning(f"Thematic summary generation skipped: {e}")
