...
DEFAULT_API_TIMEOUT_SECONDS = int(os.environ.get("OPENAI_TIMEOUT_SECONDS", "30"))


def stable_hash(*parts) -> str:
    """Create a stable cache key for repeated LLM/RAG coding calls."""
    h = hashlib.sha256()
    for part in parts:
...
def user_friendly_api_error(exc: Exception) -> str:
    """Convert OpenAI/connection errors into short messages shown in the app output."""
    msg = str(exc)
    if is_quota_error(exc):
        return (
            "OpenAI API quota/billing error. Your API key has no available quota or billing is not enabled. "
            "Reduce calls, switch to a cheaper model, or check OpenAI platform billing."
        )
    if isinstance(exc, APITimeoutError) or "timeout" in msg.lower():
        return "OpenAI API timeout. The request took too long, so this segment was skipped instead of blocking the app."
    if isinstance(exc, RateLimitError) or "rate limit" in msg.lower() or "429" in msg:
        return "OpenAI API rate limit/quota error. The app stopped this call to avoid a Streamlit crash."
    if isinstance(exc, APIError):
        return f"OpenAI API error: {msg[:500]}"
    return f"LLM call failed: {msg[:500]}"
...
def run_llm_coding_with_context(text_segment, codebook, client, retrieved_context="", source_section="", source_kind="", model_name=DEFAULT_MODEL, api_timeout_seconds=DEFAULT_API_TIMEOUT_SECONDS, use_cache=True):
    """Paper-level ENVIO coding unit.

    Design goals:
    1. Treat each input segment as exactly one evaluation unit so LLM output aligns with human CSV rows.
    2. Use No-RAG as the baseline behavior when retrieved_context is empty.
    3. Use RAG as a refinement signal when retrieved_context is available, especially for ambiguous
       environmental / structural barriers.
    4. Return one JSON object with one segment only.
    """
    code_definitions = {
        "healthcare_access": (
            "Assign when the segment explicitly mentions access to healthcare services, HIV care, "
            "testing, treatment, prevention services, harm reduction, MAT/MOUD, providers, doctors, "
            "telehealth, therapists, medication, self-tests, or care coordination. Do not assign for "
            "general education or awareness unless it directly connects to care, testing, treatment, or services."
        ),
        "social_support": (
            "Assign when the segment explicitly mentions help, support, assistance, safe space, "
            "community support, staff support, check-ins, supervision, program support, resources given "
            "to people, or interpersonal support. Do not assign for general helpfulness without actual support."
        ),
        "stigma": (
            "Assign when the segment explicitly mentions stigma, stigmatizing language, de-stigmatizing, "
            "shame, judgment, negative attitudes, drug-seeking labels, 'clean' language, or non-stigmatizing communication."
        ),
        "mental_health": (
            "Assign when the segment explicitly mentions mental health, anxiety, stress, depression, therapy, "
            "therapist, counseling, yoga, AA meetings, coping support, or emotional well-being."
        ),
        "environmental_barrier": (
            "Assign when the segment explicitly mentions structural, institutional, or environmental barriers, "
            "including lack of resources, funding issues, staffing shortages, lack of space, confidentiality limitations, "
            "process barriers, technology barriers, transportation barriers, county/jail constraints, training capacity, "
            "implementation barriers, or limited access caused by system-level conditions. This label is often missed: "
            "recover it when the text contains clear structural-barrier evidence."
        ),
    }

    has_rag = bool(str(retrieved_context).strip())
    rag_instruction = (
        "RAG is AVAILABLE. Use it as a refinement layer: first code from the segment itself, then use RAG context "
        "only to clarify ambiguous wording, recover missing context-sensitive labels, and improve consistency with the "
        "ENVIO coding definitions. RAG may help identify environmental_barrier when the segment mentions resources, "
        "funding, staffing, access limits, implementation constraints, or institutional barriers. Do not add a code based "
        "only on RAG if the current segment contains no supporting evidence."
        if has_rag else
        "RAG is NOT AVAILABLE. Produce a No-RAG baseline decision using only the current segment and the code definitions."
    )
...
Research objective:
Compare LLM-generated qualitative coding against human annotations for de-identified qualitative transcripts.
The goal is a reproducible, transparent, segment-level first-pass coding output.
...
Code definitions:
{json.dumps(code_definitions, indent=2)}

Current evaluation segment:
{text_segment}

Retrieved RAG context:
{retrieved_context}

RAG status and use rule:
{rag_instruction}

Critical evaluation-alignment rule:
Treat the current evaluation segment as exactly ONE coding unit. Do NOT split it into smaller segments.
This is required so LLM rows align with human-coded segment_index rows.

Decision procedure:
1. Read the current segment carefully.
2. Decide which labels are explicitly supported by the segment.
3. If RAG is available, use it only as secondary evidence to clarify ambiguous terms or recover a missing label that is also supported by the segment.
4. Prefer precision, but do not miss explicit structural/environmental barriers.
5. Assign zero, one, or multiple codes from the allowed codebook only.
6. Do not invent codes outside the codebook.
7. Do not code general education, awareness, or conversation as healthcare_access unless tied directly to testing, treatment, care, providers, medication, or service access.
8. Do not code general helpfulness as social_support unless actual help, support, resources, safe space, staff support, or interpersonal assistance is described.
9. If the segment mentions lack of resources, funding, staffing, space, confidentiality, process barriers, technology limitations, transportation, jail/county constraints, or implementation capacity, strongly consider environmental_barrier.
10. Output ONLY valid JSON. No markdown. No extra explanation.

Required JSON schema. Return exactly one item in "segments":
{{
  "segments": [
    {{
      "text": "the original current evaluation segment",
      "codes": ["code1", "code2"],
      "rationale": "brief reason grounded in the segment; mention RAG only if it clarified the decision",
      "evidence": "short quote from the current segment",
...
'''
    ...
    cache_key = stable_hash(model_name, text_segment, retrieved_context, source_section, source_kind, codebook)
    if use_cache:
        if "llm_coding_cache" not in st.session_state:
            st.session_state["llm_coding_cache"] = {}
        if cache_key in st.session_state["llm_coding_cache"]:
            return st.session_state["llm_coding_cache"][cache_key]
    ...
    try:
            ...
            messages=[
                {
                    "role": "system",
                    "content": (
                        "You are a careful qualitative coding assistant. Return strict JSON only. "
                        "Treat each input as one evaluation unit. Use only the allowed codebook. "
                        "Prioritize explicit evidence and consistent coding definitions."
                    ),
                },
        ...
        error_text = user_friendly_api_error(e)
        safe_result = [{
            "text": text_segment,
            "codes": ["ERROR"],
        ...
        if use_cache and not is_quota_error(e):
            st.session_state["llm_coding_cache"][cache_key] = safe_result
        return safe_result
    ...
    try:
        ...
        if not segments:
            return [{
                "text": text_segment,
                "codes": [],
                "rationale": "No supported code was assigned.",
                "evidence": "",
                "rag_used": has_rag,
                "source_section": source_section,
                "source_type": source_kind,
        ...
        # Force exactly one returned row to preserve segment-level alignment with human annotations.
        if len(segments) == 1:
            seg = segments[0]
            seg["text"] = text_segment
            seg["codes"] = [c for c in clean_code_list(seg.get("codes", [])) if c in codebook]
            seg["source_section"] = source_section
            seg["source_type"] = source_kind
            seg["rag_used"] = has_rag
            return [seg]
        ...
        for seg in segments:
            for c in clean_code_list(seg.get("codes", [])):
                if c in codebook and c not in merged_codes:
                    merged_codes.append(c)
            if seg.get("rationale"):
                rationales.append(str(seg.get("rationale")))
            if seg.get("evidence"):
                evidence.append(str(seg.get("evidence")))

        return [{
            "text": text_segment,
            "codes": merged_codes,
            "rationale": " | ".join(rationales[:3]) or "Merged multiple model-returned units into one evaluation segment.",
...
max_chunks_per_source = st.sidebar.slider(
    "Maximum coding units per source",
    min_value=1,
    max_value=100,
    value=15,
    help="Limits LLM calls to prevent quota exhaustion and Streamlit WebSocket timeout. Increase only after testing."
)

api_timeout_seconds = st.sidebar.slider(
    "OpenAI timeout per coding unit (seconds)",
    min_value=10,
    max_value=120,
    value=DEFAULT_API_TIMEOUT_SECONDS,
    help="If one LLM call takes longer than this, the app skips that unit instead of hanging."
)

model_name = st.sidebar.text_input(
    "OpenAI model",
    value=DEFAULT_MODEL,
    help="Use gpt-4o-mini for low cost. You can override with OPENAI_MODEL or type another model here."
)
...
if st.button("Run"):
    ...
    # Load human coding once near the start so it can be used both for
    # fair segment-level evaluation and for optional human-segment-guided coding.
    human_eval_df = None
    if human_coding_file is not None:
        try:
            human_coding_file.seek(0)
            human_eval_df = pd.read_csv(human_coding_file)
            human_eval_df.columns = [str(c).strip().lstrip("\ufeff") for c in human_eval_df.columns]
            if "segment_id" in human_eval_df.columns and "segment_index" not in human_eval_df.columns:
                human_eval_df = human_eval_df.rename(columns={"segment_id": "segment_index"})
            if "segment_index" in human_eval_df.columns:
                human_eval_df["segment_index"] = pd.to_numeric(human_eval_df["segment_index"], errors="coerce")
                human_eval_df = human_eval_df.dropna(subset=["segment_index"]).copy()
                human_eval_df["segment_index"] = human_eval_df["segment_index"].astype(int)
                human_eval_df = human_eval_df.sort_values("segment_index")
            human_coding_file.seek(0)
        except Exception as e:
            st.warning(f"Could not pre-load human coding CSV: {e}")
            human_eval_df = None
    ...
    if mode == "coding":
        ...
        estimated_units = 0
        for _src in source_records:
            if human_eval_df is not None and "text" in human_eval_df.columns:
                estimated_units += min(len(human_eval_df), max_chunks_per_source)
            else:
                estimated_units += min(len(chunk_text(_src["source_text"])), max_chunks_per_source)
        estimated_llm_calls = estimated_units * (2 if run_rag_comparison else 1)
        st.info(
            f"Estimated LLM calls for this run: about {estimated_llm_calls}. "
            "Keep this small while testing to avoid quota and WebSocket timeout."
        )
        ...
        for s_idx, src in enumerate(source_records, start=1):
            ...
            # PhD-level evaluation alignment:
            # If the uploaded human coding CSV includes a text column, use those human-coded
            # segments as the coding units. This avoids artificial F1/Kappa loss caused by
            # LLM-created segmentation not matching human segment_index rows.
            use_human_segment_units = False
            if human_eval_df is not None and "text" in human_eval_df.columns:
                human_units = human_eval_df.copy()
                if "participant_id" in human_units.columns:
                    participant_mask = human_units["participant_id"].astype(str) == str(participant_id)
                    if participant_mask.any():
                        human_units = human_units[participant_mask].copy()
                chunks = human_units.sort_values("segment_index")["text"].fillna("").astype(str).tolist()
                chunks = [c for c in chunks if c.strip()]
                use_human_segment_units = bool(chunks)
            else:
                chunks = chunk_text(text)

            if not chunks:
                st.warning(f"No usable coding units found for {source_name}.")
                continue
            ...
            if len(chunks) > max_chunks_per_source:
                ...
                st.warning(
                    f"Limiting {source_name} from {original_chunk_count} coding units to "
                    f"{max_chunks_per_source} to avoid API quota exhaustion and WebSocket timeout."
                )

            if use_human_segment_units:
                st.info(
                    f"Using {len(chunks)} human-coded text segments as evaluation units for {source_name}. "
                    "This improves LLM-human alignment by avoiding segmentation mismatch."
                )
            ...
            def run_single_coding_setting(use_rag_flag, setting_name):
                ...
                for c_idx, chunk in enumerate(chunks, start=1):
                    status_box.info(
                        f"Processing {setting_name}: chunk {c_idx}/{len(chunks)} for {safe_filename(str(source_name))[:60]}"
                    )
                    source_section = (
                    ...
                    rag_query = (
                        "ENVIO qualitative coding guidance and code definitions for HIV care engagement. "
                        "Focus on distinguishing healthcare_access, social_support, stigma, mental_health, "
                        "and environmental_barrier. Retrieve context about structural barriers such as lack of "
                        "resources, funding, staffing, space, confidentiality, process barriers, technology, "
                        "transportation, and jail/county implementation constraints. "
                        f"Current segment: {chunk[:700]}"
                    )
                    ...
                    try:
                        ...
                        if coded_segments and "quota" in str(coded_segments[0].get("rationale", "")).lower():
                            st.error("Stopping this coding setting because OpenAI quota/billing failed. Fix billing or reduce calls, then rerun.")
                            break
                    except Exception as e:
                        st.error(
        ...
        if all_outputs:
            ...
            if human_coding_file is not None:
                try:
                    human_df = human_eval_df.copy() if human_eval_df is not None else pd.read_csv(human_coding_file)
