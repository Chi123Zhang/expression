...
def refine_with_rag_context(text_segment, codebook, client, retrieved_context="", source_kind=""):
    """Convert retrieved RAG context into compact, structured coding guidance.

    This function is used only in the RAG branch. It keeps No-RAG as the clean baseline
    while letting RAG act as a refinement layer instead of simply adding raw context.
    """
    ...
    prompt = f"""
You are preparing coding guidance for an ENVIO / TechMPower qualitative coding task.

Your task is NOT to assign final codes yet. Your task is to convert the retrieved RAG context into concise coding guidance for the current segment.
...
{{
  "key_themes": ["brief themes relevant to the current segment"],
  "coding_hints": ["specific guidance about which codes may be supported and why"],
  "risk_of_false_positive": ["codes that should NOT be over-assigned"],
  "when_not_to_code": ["situations where no code should be assigned"]
}}
"""
    ...
    response = client.chat.completions.create(
        ...
        temperature=0,
        messages=[
            {"role": "system", "content": "Return strict JSON only. Do not assign final codes."},
    ...
    try:
        ...
        return {
            "key_themes": [],
            "coding_hints": ["RAG refinement could not be parsed; rely primarily on the segment."],
            "risk_of_false_positive": [],
            "when_not_to_code": [],
            "raw_refinement": content,
        }


def run_llm_coding_with_context(text_segment, codebook, client, retrieved_context="", source_section="", source_kind="", refinement=None):
    refinement_text = ""
    if refinement:
        refinement_text = f"""
Structured RAG refinement guidance:
...
6. If structured RAG refinement guidance is provided, use it as a secondary refinement layer: it may clarify ambiguous meaning, but it must not override the current segment.
7. Do not assign a code based only on retrieved context or refinement guidance. The current segment itself must contain supporting evidence.
8. Prefer precision over recall: avoid weak or speculative labels.
...
{{
  "segments": [
    {{
      ...
      "codes": ["code1", "code2"],
      "rationale": "brief reason grounded in the text; mention RAG refinement only if it changed or clarified the decision",
        ...
        for s_idx, src in enumerate(source_records, start=1):
            ...
            def run_single_coding_setting(use_rag_flag, setting_name):
                ...
                for c_idx, chunk in enumerate(chunks, start=1):
                    ...
                    try:
                        refinement = None
                        if use_rag_flag and str(retrieved_context).strip():
                            refinement = refine_with_rag_context(
                                text_segment=chunk,
                                codebook=CODEBOOK,
                                client=client,
