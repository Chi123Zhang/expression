...
# ---------------------------------------------------------------------------
# Lightweight thematic summary expression layer
# ---------------------------------------------------------------------------
def generate_role_adaptive_thematic_summary(
    ...
    """Rewrite a neutral thematic summary for the active user role.

    This is intentionally lightweight: it does not rerun coding, retrieval,
    topic modeling, or evaluation. It only reformats the final thematic summary
    so different users can receive different interpretations of the same coded
    evidence.

    Args:
        thematic_summary: Neutral qualitative summary generated after coding.
        profile: Active profile from the app, for example
            {"role": "business", "technical_level": "medium", "goal": "understanding"}.
        retrieved_context: Optional short transcript/context excerpt used only
            for grounding. Keep this short to avoid extra latency.
        user_question: Optional user-entered research question or instruction.
        model: Model used for the final lightweight rewrite.

    Returns:
        Role-adaptive thematic interpretation.
    """

    if not thematic_summary:
        return "No thematic summary is available yet."

    profile = profile or {}
    role = _normalize_role(profile.get("role") or profile.get("target_audience") or "general")
    technical_level = str(profile.get("technical_level", profile.get("technical_depth", "medium")))
    goal = str(profile.get("goal", "understanding"))

    # Keep this role logic intentionally compact. The deeper expression plan
    # machinery above remains unchanged for full project/document answers.
    role_guidance = {
        "general": "Explain the major themes clearly and avoid unnecessary jargon. Focus on what the interview says and why it matters.",
        "business": "Focus on operational implications, service gaps, resource needs, implementation risks, and practical next steps. Avoid technical model details.",
        "business_owner": "Focus on decision-relevant takeaways, operational value, risks, and what leadership should do next. Avoid technical model details.",
        "pm": "Focus on user needs, workflow implications, feature opportunities, risks, and success criteria for a research-support tool.",
        "product_manager": "Focus on user needs, workflow implications, feature opportunities, risks, and success criteria for a research-support tool.",
        "engineer": "Focus on system behavior, retrieval/coding implications, evaluation signals, failure modes, and implementation next steps.",
        "researcher": "Focus on qualitative themes, interpretation, coding consistency, evidence patterns, and research implications.",
        "qualitative_researcher": "Focus on qualitative themes, interpretation, coding consistency, evidence patterns, and research implications.",
        "clinician": "Focus on participant needs, care access, stigma reduction, support systems, continuity of care, and practical service implications.",
        "social_worker": "Focus on support needs, barriers, service navigation, trust-building, stigma reduction, and practical care coordination.",
    }
    guidance = role_guidance.get(role, role_guidance["general"])

    if OpenAI is None or not os.getenv("OPENAI_API_KEY"):
        return _fallback_role_adaptive_thematic_summary(
    ...
    prompt = f"""
You are the final expression layer for an AI-assisted qualitative research system.

Your task is to rewrite a neutral thematic summary for the active user role.
Do not rerun coding. Do not invent new findings. Stay faithful to the summary and any provided context.
...
Neutral thematic summary:
{thematic_summary}

Optional grounding context:
{(retrieved_context or "Not provided")[:3000]}

Write the final answer with this structure:
1. Main takeaway
2. Major themes
3. Why this matters for the user's role
4. Practical next step

Keep it concise, grounded, and natural.
""".strip()
    ...
    try:
        response = client.chat.completions.create(
            model=model,
            messages=[
                {"role": "system", "content": "You are a careful qualitative research assistant. Do not invent unsupported facts."},
                {"role": "user", "content": prompt},
            ],
            temperature=0.3,
        )
        return response.choices[0].message.content or _fallback_role_adaptive_thematic_summary(
            thematic_summary=thematic_summary,
            role=role,
            user_question=user_question,
        )
...
def _fallback_role_adaptive_thematic_summary(
    thematic_summary: str,
    role: str = "general",
    user_question: Optional[str] = None,
) -> str:
    """Deterministic fallback for role-adaptive thematic summaries."""

    role = _normalize_role(role)
    question_line = f"\n\nUser question: {user_question}" if user_question else ""

    if role in {"business", "business_owner"}:
        return (
            "1. Main takeaway\n"
            "The interview points to practical service and operational challenges that may affect access, support, and implementation."
            f"{question_line}\n\n"
            "2. Major themes\n"
            f"{thematic_summary}\n\n"
            "3. Why this matters for this role\n"
            "These themes can inform decisions about service design, staffing, resource allocation, and implementation priorities.\n\n"
            "4. Practical next step\n"
            "Review the coded segments with the highest relevance to access barriers and support needs before translating findings into program decisions."
        )

    if role in {"engineer"}:
        return (
            "1. Main takeaway\n"
            "The thematic summary reflects the downstream interpretation produced after transcript coding and contextual retrieval."
            f"{question_line}\n\n"
            "2. Major themes\n"
            f"{thematic_summary}\n\n"
            "3. Why this matters for this role\n"
            "The summary layer helps validate whether retrieval and coding outputs produce coherent transcript-level themes rather than only segment-level labels.\n\n"
            "4. Practical next step\n"
            "Compare the summary against retrieved segments, code frequencies, and human-aligned evaluation metrics to identify failure modes."
        )

    if role in {"pm", "product_manager"}:
        return (
            "1. Main takeaway\n"
            "The interview summary shows what a researcher or practitioner would need to understand from the transcript after coding."
            f"{question_line}\n\n"
            "2. Major themes\n"
            f"{thematic_summary}\n\n"
            "3. Why this matters for this role\n"
            "This helps define the user value of the system: turning transcript coding outputs into interpretable research findings.\n\n"
            "4. Practical next step\n"
            "Use this output to refine the product workflow around transcript upload, coding review, evidence checking, and exportable reports."
        )

    return (
        "1. Main takeaway\n"
        "The interview contains several recurring qualitative themes that can be summarized after coding."
        f"{question_line}\n\n"
        "2. Major themes\n"
        f"{thematic_summary}\n\n"
        "3. Why this matters\n"
        "This turns coded segments into a clearer transcript-level interpretation.\n\n"
        "4. Practical next step\n"
        "Review the most relevant coded excerpts to confirm that the summary is grounded in the transcript."
    )
